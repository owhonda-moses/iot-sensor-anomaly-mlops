...
baseline_params = {
    "if_n_estimators": 100,
    "if_max_samples": "auto",
    "contamination": 0.01,  # IF
    "ae_encoding_dim": 16,
    "ae_lr": 1e-3,
    "ae_epochs": 30,
    "ae_batch_size": 128,  # AE
    "w_if": 1,
    "w_ae": 0,
    "thr_q": 0.95,  # fuse & threshold
}
best_usl = {
    "if_n_estimators": 148,
    "if_max_samples": 1.0,
...
def engineer_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["device"] = df["device"].map({k: f"device_{i+1}" for i, k in enumerate(env_map)})
    ...
    for dev in df["device"].unique():
        ...
        qt[dev] = dict(
            temp=(math.floor(lo[0]), math.floor(hi[0])),
...
def preprocess(
    ...
    rolling_feats = [
        f"{c}_{agg}" for c in SENSOR_COLS for agg in ["roll_mean", "roll_std"]
    ]
    ...
    X_df = pd.concat(
        [df[base_feats + rolling_feats + cyclic_feats], env_ohe, dev_ohe], axis=1
    )
...
def train_sl(X_tr, y_tr, X_te, y_te, C, tol, penalty, random_state, n_splits=5):
    ...
    # aggregate
    avg_metrics = {
        k: round(np.mean([m[k] for m in fold_metrics]), 3) for k in fold_metrics[0]
    }
...
def main():
    ...
    if args.run_optuna_sl:  # SL tuning
        ...
        study.optimize(
            ...
            callbacks=[
                lambda s, t: (
                    pbar.update(1),
                    pbar.set_postfix(best_f1=f"{s.best_value:.4f}"),
                )
            ],
        )
        pbar.close()
    ...
    sl_opt = train_sl(
        X_tr=Xtr, y_tr=ytr, X_te=Xte, y_te=yte, **sl_best, random_state=seed, n_splits=5
    )
    ...
    save_artifacts(
        usl_base=usl_base,
        sl_base=sl_base,
        sl_base_params=sl_base_params,
        usl_opt=usl_opt,
        sl_opt=sl_opt,
        sl_opt_params=sl_best,
        scaler=scaler,
        X_cols=X_cols,
        model_dir=model_dir,
    )
